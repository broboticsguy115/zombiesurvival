...
function GM:HumanHUD(screenscale)
	...
	if not self.RoundEnded then
		if self:GetWave() == 0 and not self:GetWaveActive() then
			...
			draw_SimpleTextBlurry(translate.Format("hint_starting_gear", input.LookupBinding("gm_showteam") or "F2"), "ZSHUDFontSmall", w * 0.5, h * 0.25, COLOR_GRAY, TEXT_ALIGN_CENTER)
			draw_SimpleTextBlurry(translate.Get("round_begins").." "..util.ToMinutesSecondsCD(math.max(0, self:GetWaveStart() - curtime)), "ZSHUDFontSmall", w * 0.5, h * 0.28, COLOR_GRAY, TEXT_ALIGN_CENTER)
	...
	if gamemode.Call("PlayerCanPurchase", MySelf) then
		if self:GetWaveActive() then
			draw_SimpleTextBlurry(translate.Format("press_f2_for_the_points_shop", input.LookupBinding("gm_showteam") or "F2"), "ZSHUDFontSmall", w * 0.5, 8, COLOR_GRAY, TEXT_ALIGN_CENTER)
		else
			local th = draw_GetFontHeight("ZSHUDFontSmall")
			draw_SimpleTextBlurry(translate.Format("press_f2_for_the_points_shop", input.LookupBinding("gm_showteam") or "F2"), "ZSHUDFontSmall", w * 0.5, 8, COLOR_GRAY, TEXT_ALIGN_CENTER)
			draw_SimpleTextBlurry(translate.Format("x_discount_for_buying_between_waves", 20), "ZSHUDFontSmall", w * 0.5, 9 + th, COLOR_GRAY, TEXT_ALIGN_CENTER)
		end
	end
end
...
function GM:Create3DFonts()
	...
	surface.CreateLegacyFont(fontfamily, 22 + fontsizeadd3D, fontweight3D, false, false,  "ZS3D2DFont2Tiny", false, true)
...
function GM:HUDPaintBackgroundEndRound()
	...
	if timleft <= 0 then
		...
		draw_SimpleText(translate.Get("loading"), "ZSHUDFontSmaller", 50, 27, COLOR_GRAY)
	else
		draw_SimpleText(translate.Format("next_round_in_x", util.ToMinutesSecondsCD(timleft)), "ZSHUDFontSmaller", 50, 27, 10 < timleft and COLOR_GRAY or Color(255, 0, 0, math.abs(math.sin(RealTime() * 8)) * 180 + 40))
		draw_SimpleTextBlur(translate.Format("next_round_in_x", util.ToMinutesSecondsCD(timleft)), "ZSHUDFontSmall", x, y, COLOR_GRAY, TEXT_ALIGN_CENTER)
	end
end

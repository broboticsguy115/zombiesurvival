...
concommand.Add("zs_pointsshopbuy", function(sender, command, arguments)
	...
	if usescrap and not sender:NearRemantler() or not usescrap and not sender:NearArsenalCrate() then
		if sender.ArsenalBan then
			GAMEMODE:ConCommandErrorMessage(sender, translate.ClientGet(sender, usescrap and "banned_for_life_warning"))
		else
			GAMEMODE:ConCommandErrorMessage(sender, translate.ClientGet(sender, usescrap and "need_to_be_near_arsenal_crate"))
		end
		return
	end
	...
	if GAMEMODE:GetWaveActive() == true then
		cost = usescrap and math.ceil(GAMEMODE:PointsToScrap(cost)) or math.floor(cost * 1)
	else
		cost = usescrap and math.ceil(GAMEMODE:PointsToScrap(cost)) or math.floor(cost * 0.80)
	end

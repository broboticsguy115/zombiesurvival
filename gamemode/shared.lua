...
function GM:PlayerCanPurchase(pl)
	...
	local canpurchase = PTeam(pl) == TEAM_HUMAN and self:GetWave() > 0 and pl:Alive() and pl:NearArsenalCrate() and not pl.ArsenalBan
...
function GM:GetBotDamageScale(pos, ignore) --Helps prevent wave 1 steamrolls on servers with little or no players.
	if humans > 6 then
		return self.ZombieDamageMultiplier * (1 - self:GetDamageResistance(self:GetFearMeterPower(pos, TEAM_UNDEAD, ignore)))
	elseif humans == 6 then
		return self.ZombieDamageMultiplier + 0.1 * (1 - self:GetDamageResistance(self:GetFearMeterPower(pos, TEAM_UNDEAD, ignore)))
	elseif humans == 5 then
		return self.ZombieDamageMultiplier + 0.2 * (1 - self:GetDamageResistance(self:GetFearMeterPower(pos, TEAM_UNDEAD, ignore)))
	elseif humans == 4 then
		return self.ZombieDamageMultiplier + 0.4 * (1 - self:GetDamageResistance(self:GetFearMeterPower(pos, TEAM_UNDEAD, ignore)))
	elseif humans == 3 then
		return self.ZombieDamageMultiplier + 0.6 * (1 - self:GetDamageResistance(self:GetFearMeterPower(pos, TEAM_UNDEAD, ignore)))
	elseif humans == 2 then
		return self.ZombieDamageMultiplier + 0.8 * (1 - self:GetDamageResistance(self:GetFearMeterPower(pos, TEAM_UNDEAD, ignore)))
	elseif humans <= 1 then
		--print("Hello? Is this thing on? Am I all alone?")
		return self.ZombieDamageMultiplier + 1 * (1 - self:GetDamageResistance(self:GetFearMeterPower(pos, TEAM_UNDEAD, ignore)))
	end
end

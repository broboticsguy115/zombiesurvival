...
function GM:TryHumanPickup(pl, entity)
	...
	if gamemode.Call("DisallowHumanPickup", pl, entity) or pl:GetInfo("zs_nopickupprops") == "1" or pl.DebuffNoodlearms then return end
...
function GM:ShowTeam(pl)
	if pl:Team() == TEAM_HUMAN and not self.ZombieEscape and not pl.ArsenalBan then
		pl:SendLua(self:GetWave() > 0 and "GAMEMODE:OpenArsenalMenu()" or "MakepWorth()")
	elseif pl:Team() == TEAM_HUMAN and pl.ArsenalBan then
		pl:CenterNotify(COLOR_RED, translate.ClientGet(pl, "banned_for_life_warning"))
	end
end
...
function GM:ShowSpare1(pl)
	...
	elseif pl:Team() == TEAM_HUMAN then
		--pl:SendLua("GAMEMODE:ToggleSkillWeb()") No skillweb! This is not an RPG JetBoom. It's a round based team game.
	end
end
	...
	if NextTick <= time then
		...
		for _, pl in pairs(allplayers) do
			if P_Team(pl) == TEAM_HUMAN and P_Alive(pl) then
				...
				--Trinket/skill stuff. I'd remove the code for it, but I don't want to break stuff. Players can't get into the skill menu anyways.
...
function GM:CanRemoveOthersNail(pl, nailowner, ent)
	...
	if pl:GetZSLevel() < nailowner:GetZSLevel() then

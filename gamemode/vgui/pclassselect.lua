...
hook.Add("PlayerInitialSpawnRound", "CloseOnRoundEnd", function(pl)	
	if Window and Window:IsValid() then
		Window:Remove()
	end
	hook.Remove("PlayerInitialSpawnRound", "CloseOnRoundEnd")
end)
...
function PANEL:Init()
	...
	for i = 1, #GAMEMODE.ZombieClasses do
		...
		if classtab and not classtab.Hidden and not classtab.Boss or classtab.CanUse and classtab:CanUse(MySelf) then
...
function PANEL:Init()
	...
	for i = 1, #GAMEMODE.ZombieClasses do
		local classtab = GAMEMODE.ZombieClasses[i]
		if classtab and classtab.Boss and not classtab.Hidden then
			local button = vgui.Create("ClassButton", self)
			button:SetClassTable(classtab)

...
function meta:ProcessDamage(dmginfo)
	...
	if P_Team(self) == TEAM_UNDEAD then
		...
		if self ~= attacker and not corrosion and not dmgbypass then
			if self:IsBot() then
				dmginfo:SetDamage(dmginfo:GetDamage() * GAMEMODE:GetBotDamageScale(dmginfo:GetDamagePosition(), self))
			else
	...
	if attacker:IsValid() and attacker:IsPlayer() and inflictor:IsValid() and attacker:Team() == TEAM_UNDEAD then
		if inflictor == attacker:GetActiveWeapon() then
			...
			if self.DebuffBleed and (damage >= 4 and dmgtype == 0 or bit.band(dmgtype, DMG_TAKE_BLEED) ~= 0) then
...
function meta:AddPoisonDamage(damage, attacker)
	...
	if damage > 0 then
		...
		if status and status:IsValid() then
			if self.BuffResist then
				status:AddDamage(damage / 2, attacker)
			else
...
function meta:DoNoodleArmBones()
	if self.DebuffNoodlearms and self:Team() == TEAM_HUMAN then
		self.NoodleArmBones = {}

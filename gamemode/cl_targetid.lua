...
function GM:DrawTargetID(ent, fade)
	...
	if ent:Team() == TEAM_UNDEAD then
		...
		if remortlevel >= 1 then
			draw.SimpleTextBlur(string_format("Level %d", level), "ZSHUDFontTiny", x, y, colTemp, TEXT_ALIGN_CENTER)
		else
			draw.SimpleTextBlur("Level "..level, "ZSHUDFontTiny", x, y, colTemp, TEXT_ALIGN_CENTER)
		end
	end
end

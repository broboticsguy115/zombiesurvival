...
function SWEP:PrimaryAttack()
	...
	if totake > 0 then
		if owner.BuffMedkit then
			self:SetNextCharge(CurTime() + self.Primary.Delay * math.min(1, healed / self.Heal) * 0.70)
		else
...
function SWEP:SecondaryAttack()
	...
	if totake > 0 then
		if owner.BuffMedkit then
			self:SetNextCharge(CurTime() + self.Primary.Delay * self.Secondary.DelayMul * math.min(1, healed / self.Heal * self.Secondary.HealMul) * 0.70)
		else
